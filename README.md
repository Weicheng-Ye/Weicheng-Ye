# Hi, I am Weicheng Ye 👋

🔭 I am currently a postdoctoral fellow at the Chinese University of Hong Kong. I was trained as a theoretical and mathematical physicist at Peking University and Perimeter Institute for Theoretical Physics.

## 👀 Current interests

* Reinforcement learning and its applications in mathematics and quantum physics
* Agentic systems for mathematics, physics, and quantitative research
* Mathematical physics

## 🗂️ Repositories by theme

### 🌐 Personal

* [**Weicheng-Ye.github.io**](https://github.com/Weicheng-Ye/Weicheng-Ye.github.io) — my personal academic website.

### 🌀 Mathematical Physics

* [**fermionAHSS**](https://github.com/Weicheng-Ye/fermionicAHSS) — a GAP package computing Atiyah–Hirzebruch spectral sequence (AHSS) pages through E6 for studying fermionic symmetry-protected topological phases up to (5+1) dimensions.
* [**QSLClassification**](https://github.com/Weicheng-Ye/QSLClassification) — computational classification of symmetry-enriched topological quantum spin liquids.
* [**mathPSG**](https://github.com/Weicheng-Ye/mathPSG) — projective symmetry group classifications for occupied Wyckoff positions in 3D space groups. *(collaborative development with Chunxiao Liu)*
* [**SpaceGroupCohomology**](https://github.com/Weicheng-Ye/SpaceGroupCohomology) — mod-2 cohomology rings and Lieb–Schultz–Mattis anomaly classes for the 230 crystallographic space groups. *(collaborative development with Chunxiao Liu)*
* [**umtc**](https://github.com/Weicheng-Ye/umtc) — UMTC data, intrinsic symmetries, and consistency checks for anyon models.
* [**Classification-of-Stiefel-Liquid**](https://github.com/Weicheng-Ye/Classification-of-Stiefel-Liquid) — code accompanying [arXiv:2111.12097](https://arxiv.org/abs/2111.12097).

### 🤖 AI for mathematics & agents

* [**Agon**](https://github.com/AutoResearch-Factory/Agon) — an autonomous, large-scale research system for taking research ideas from exploration to computational experiments. *(fork)*
* [**AgonAlpha**](https://github.com/AutoResearch-Factory/AgonAlpha) — an agentic framework for autonomous and reproducible quantitative alpha discovery. *(collaborative development with Agon Team)*
* [**MathBenchmark**](https://github.com/Weicheng-Ye/MathBenchmark) — a mathematical benchmark for LLMs and agents, with Lean-based formalization.
* [**Rethlas**](https://github.com/Weicheng-Ye/Rethlas) — experiments with an AI-driven proof system. *(fork)*

## 💬 Beyond research

I love traveling and meeting new friends! Let's chat when we are in the same city!
